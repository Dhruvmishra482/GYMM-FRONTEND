import React, { useRef, useEffect, useState, useMemo, useCallback, memo, lazy, Suspense } from "react";
...
const StatCard = memo(({ icon: Icon, title, value, gradient, iconGradient }) => (
  <div className={`p-6 transition-all duration-300 border ${gradient} rounded-lg shadow-md hover:shadow-lg hover:scale-105`}>
    <div className="flex items-center">
      <div className={`p-3 rounded-lg shadow-md ${iconGradient}`}>
        <Icon className="w-6 h-6 text-white" />
      </div>
      <div className="ml-4">
        <p className="text-sm font-medium text-gray-600">{title}</p>
        <p className="text-2xl font-bold text-transparent bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text">{value}</p>
      </div>
    </div>
  </div>
));
...
const MemberList = ({
  ...
  // Optimized filter with useMemo - prevents unnecessary recalculations
  ...
  const { totalPages, startIndex, paginatedMembers } = useMemo(() => {
    ...
    const paginated = filteredAndSortedMembers.slice(start, start + ITEMS_PER_PAGE);
