...
import { Edit, Trash2, Eye, Send, Target, CalendarDays, Zap } from 'lucide-react';
...
const DietPlanCard = ({ plan, onBroadcast, onEdit, onPreview, onDelete }) => {
  ...
  return (
    <div
      className="rounded-2xl border border-slate-300 bg-gradient-to-br from-slate-50 to-emerald-50 p-5 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg cursor-pointer"
      ...
      <div className="flex h-full flex-col">
        <div className="flex-grow">
          ...
          <div className="mb-4 space-y-2 text-sm text-slate-700">
            <p className="flex items-center gap-2"><Zap className="h-4 w-4 text-emerald-600" />Plan Type: <span className="font-medium text-slate-900">{plan.planType}</span></p>
            <p className="flex items-center gap-2"><Target className="h-4 w-4 text-teal-600" />Audience: <span className="font-medium text-slate-900">{plan.targetAudience}</span></p>
