// ResetPasswordUI.jsx - Enhanced with Lazy Loading + Advanced Memoization + useTransition
import React, { 
  useState, 
  useEffect, 
  useCallback, 
...
const PasswordFormSection = lazy(() => Promise.resolve({
  default: memo(({ 
    formData,
    showPassword,
    showConfirmPassword,
...
const SuccessSection = lazy(() => Promise.resolve({
  default: memo(({ message, onGoToLogin }) => (
...
const ErrorMessages = lazy(() => Promise.resolve({
  default: memo(({ error, validationErrors }) => (
...
const ResetPasswordUI = memo(({ 
  ...
  const passwordsMatch = useMemo(() => formData.newPassword === formData.confirmPassword && formData.confirmPassword, [formData.newPassword, formData.confirmPassword]);
          ...
          <div className="p-6">
            ...
            {success ? (
              <Suspense fallback={<FormSkeleton />}>
                <SuccessSection message={message} onGoToLogin={handleGoToLogin} />
              </Suspense>
            ) : (
              <div className="space-y-6">
                ...
                {/* Error Messages - Lazy loaded */}
                {hasErrors && (
                  <Suspense fallback={<div className="h-16 bg-red-50 rounded-lg animate-pulse"></div>}>
                    <ErrorMessages error={error} validationErrors={validationErrors} />
                  </Suspense>
                )}

                {/* Password Form - Lazy loaded */}
                <Suspense fallback={<FormSkeleton />}>
                  <PasswordFormSection
                    formData={formData}
