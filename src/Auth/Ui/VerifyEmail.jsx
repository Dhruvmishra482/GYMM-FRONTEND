...
const OTPInputSection = lazy(() =>
  Promise.resolve({
    default: memo(({ otp, onOTPChange, onSubmit, isSubmitting }) => (
...
const TimerSection = lazy(() =>
  Promise.resolve({
    default: memo(
      ({ canResend, timeLeft, formatTime, onResendOTP, isResending }) => (
...
const VerifyEmailForm = memo(
  ({
    ...
    const handleOTPChangeWithTransition = useCallback(
      (value) => {
    ...
    const handleSubmitWithTransition = useCallback(
      (e) => {
        e.preventDefault();
        handleSubmit(e);
      },
    ...
    // REMOVED: All duplicate function definitions that conflicted with props
    // - formatTime (now using prop)
    // - handleOTPChange (now using prop)
    // - handleSubmit (now using prop)
    // - handleResendOTP (now using prop)

    // REMOVED: useEffect that tried to call setTimeLeft/setCanResend
    // These state updates should be handled in the parent component
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex items-center justify-center p-6 relative overflow-hidden">
        ...
        <div className={containerClassName}>
          ...
          <div className="bg-gradient-to-r from-white to-blue-50 rounded-xl shadow-md border border-blue-100 p-6 mb-6 text-center hover:shadow-lg transition-all duration-300">
            ...
            <p className="text-blue-600 font-medium">{emailDisplay || email}</p>
          </div>
